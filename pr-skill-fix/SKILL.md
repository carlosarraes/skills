---
name: pr-skill-fix
description: "Handle HUMAN reviewer feedback on every open non-draft PR by the user — distinct from `pr-sweep` (which is bots only). Classifies each PR by review state and decides: APPROVED + non-blocking comments → score each finding (trivial = fix-here, scope-creep = follow-up ticket) and confirm the batch with the user once before acting, because pushing more commits invalidates the approval; CHANGES_REQUESTED + overall body → fix silently, one commit per finding, no reply to the reviewer; CHANGES_REQUESTED + inline threads → one commit per thread, reply with the SHA, resolve the thread (same as the bot protocol). After fixes land, sleeps 10 min via `ScheduleWakeup`, rechecks CI + watches for new bot activity, and re-requests review from the humans who set CHANGES_REQUESTED — leaving a single PR-level summary comment so the reviewer has context. Linear (default) or Jira (`jira` arg) for follow-up tickets. Use this skill whenever the user says 'fix user comments on my PRs', 'address review feedback', 'handle changes requested', 'reply to my reviewer', 'fix the human comments', 'respond to PR review', 'work through my review feedback', 'fix and re-request review', '/pr-skill-fix', or has just received a CHANGES_REQUESTED review (or an approval with nits) and wants automated handling until the PR is back in front of the reviewer. Also trigger on 'address PR feedback', 'fix reviewer nits', 'turn around the review comments', or whenever the user is staring at a wall of human review threads and wants someone to drive them to resolution."
---
...
```
PROJ-877 → /Users/carraesmb/acme/example-platform-proj-877-a11y
```

If no map is given, **auto-derive** the worktree path from the branch name following the `example-platform-<branch>` convention. If the path doesn't exist, `git worktree add <path> <branch>` against the remote tip before applying any fix. Never reuse a worktree that's currently on a different branch.
