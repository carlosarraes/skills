---
name: clean-up
description: "Audit a branch for correctness bugs, code-reuse misses, quality issues, efficiency problems, and missing regression tests, then fix the valid findings using TDD discipline (failing test first, one focused commit per finding) and finish with a /simplify pass. Use this skill whenever the user says 'clean up this branch', 'clean up PROJ-883', 'clean up <ticket>', '/clean-up', 'audit this branch', 'review and refactor my branch', 'review and clean my code', 'sanity-check this branch before PR', 'tidy up before merge', 'lint and refactor my changes', 'find latent bugs in my branch', or wants a multi-agent review-then-fix pass over the diff. Triggers on the pattern of vibe-coded branches that need a senior-engineer pass before merge — pulls in /find-skills if a needed capability isn't yet installed, leverages existing helpers in the codebase before introducing new abstractions, and never auto-pushes or auto-opens a PR. Accepts a branch name OR a ticket ID (e.g., 'clean up feature/foo' or 'clean up PROJ-883') — if neither is given, operates on the current branch."
---
