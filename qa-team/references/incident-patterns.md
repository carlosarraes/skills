...
- Schema changes on tables shared with external services (e.g., a Django table also written to by a background worker or another service)
- Migration analyzers not recognizing non-standard app labels, bypassing safety checks
- Plain SQL migrations drifting from Django's migration state
...
- Undersized hosts causing CPU saturation >90%
...
- Deployment race conditions between old and new code during rollout
- Serialization bugs when both old and new field names exist in cached data

**Review signals:**

- Any change to hot-path evaluation, serialization, or caching logic
- Timeout/retry configuration changes
- Deploy/config value refactors affecting service routing or selectors
- Serialization boundaries between backend writers and readers
- Changes to background task memory patterns (unbounded data loading)

## Pattern 3: Backend/Frontend Compatibility Breaks

**Common triggers:**

- Frontend calling a backend field or endpoint only available after the backend deploys
- Frontend served from cache/CDN assuming the latest contract against a still-deploying backend
...
- Deploy steps gated on manual approval that get missed — fixes never deploy
- No client-side error monitoring; detection relies on support tickets

**Review signals:**

- Any change to frontend code that references backend fields or contracts
- Changes to fetch/XHR wrappers or request interception
- Backend/frontend version synchronization during a deploy window
- Missing feature flags for high-risk contract changes
- Absence of integration tests across backend/frontend version combinations
...
- Write amplification from triggers or materialized views (1 insert triggering hundreds of real writes)
- Coordination service saturation (e.g., a task broker hitting outstanding request limits)
- Untagged database queries running as a shared/default user, invisible to resource management
- Missing backpressure configuration (unbounded queue or write growth)
- Background tasks loading entire datasets into memory without batching or streaming
- CPU saturation from undersized hosts
...
- Resource/memory limit changes in deployment configs
...
- Database engine or config changes breaking aggregation logic silently
...
- Misleading metrics (e.g., workers in crash-loop not generating OOM events during idle periods)

**Review signals:**

- Database version or engine setting changes
...
- Configuration refactors creating non-atomic changes across multiple files or PRs
- Default values that fail silently (e.g., a selector or route matching nothing)
- Infrastructure migrations breaking specific services in production despite passing in staging
...
- Infrastructure changes affecting request routing or networking
...
- Jobs produced to one queue but consumed from another due to missing configuration mapping
...
- **Generic/unhelpful error messages** — the API returns a specific validation error but the UI
...
- **Ambiguous visual states** — Toggle switches, dropdowns, and inputs look the same
  ...
  real input. Different entity types not visually distinct in pickers.
...
- **Misleading or confusing text** — Date formats that prioritize less-useful information
  (e.g., "first seen" before "last seen"), error messages with internal jargon or plan/tier
  names, copy that doesn't match the actual product behavior.
...
3. **Client-side-only filtering** — relying on UI targeting instead of server-side access control
4. **Single-service testing of multi-service changes** — testing only the changed service, not downstream consumers
5. **Manual deployment steps** — approval gates, cache warming that humans forget
6. **Environment asymmetry** — configuration differences between environments causing incidents in only one
