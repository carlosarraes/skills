...
- **PR title validation failures**: the `action-semantic-pull-request` check enforces lowercase first word after the colon. If the title starts with a ticket ID (`PROJ-1096`) or an acronym (`PR-review`), it fails. Title-edit re-fires the check; no code change required.
