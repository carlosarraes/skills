...
def close_pending_ownership(module, kind, ownership):
    try:
        with module._ownership_cleanup(
            kind,
            ownership,
        ) as admission:
            closed = module._close_owned_record(kind, ownership)
            if not closed:
                admission.retain()
            return closed
    except module.SessionBusyError:
        return False
...
class AuditRuntimeReconciliationTests(unittest.TestCase):
    ...
    def test_precommit_append_failure_leaves_tombstone_recoverable(self):
        with materialized_repo(
            ...
            while session_module._TRACKED_LEASE_OWNERSHIPS:
                if time.monotonic() >= deadline:
                    self.fail("claim cleanup did not finish")
                time.sleep(0.005)
            original_state = store.load(started.session)
            next_state = {
                **original_state,
        ...
        with materialized_repo(
            ...
            with mock.patch.object(
                ...
                while (
                    ...
                    if time.monotonic() >= deadline:
                        self.fail("paused lease cleanup did not finish")
                    time.sleep(0.005)
            ...
            while (
                ...
                or session_module._TRACKED_TRANSACTION_OWNERSHIPS
            ):
                if time.monotonic() >= deadline:
                    self.fail("recovery cleanup did not finish")
                time.sleep(0.005)
    ...
    def test_paused_claim_open_does_not_block_an_independent_claim(self):
        with materialized_repo(
            ...
            while session_module._TRACKED_LEASE_OWNERSHIPS:
                if time.monotonic() >= deadline:
                    self.fail("asynchronous lease cleanup did not finish")
                time.sleep(0.005)
            self.assertEqual(
                session_module._TRACKED_LEASE_OWNERSHIPS,
                set(),
        ...
        with materialized_repo(
            ...
            with mock.patch.object(
                ...
                while (
                    ...
                    if time.monotonic() >= deadline:
                        self.fail(
                            "paused transaction cleanup did not finish"
                        )
                    time.sleep(0.005)
            ...
            while session_module._TRACKED_LEASE_OWNERSHIPS:
                if time.monotonic() >= deadline:
                    self.fail("lease cleanup did not finish")
                time.sleep(0.005)
    ...
    def test_cleanup_exclusion_precedes_reused_fd_publication(self):
        ...
        cleanup = threading.Thread(
            target=close_pending_ownership,
            args=(session_module, "lease", stale_ownership),
        )
    ...
    def test_consumed_slot_survives_forget_crash_and_same_inode_reuse(self):
        session_module = sys.modules["audit_session"]
        with tempfile.TemporaryDirectory() as temporary:
            store = self.module.SessionStore(Path(temporary))
            descriptor = store._open_directory(temporary)
            ...
            session_module._TRACKED_LEASE_OWNERSHIPS.add(ownership)
            original_forget = session_module._forget_owned_record
            replacement = []
            crashed = False

            def crash_after_slot_close(kind, value):
                nonlocal crashed
                if value == ownership and not crashed:
                    crashed = True
                    replacement.append(
                        store._open_directory(temporary)
                    )
                    ...
                    raise KeyboardInterrupt(
                        "bookkeeping interrupted after slot close"
                    )
                return original_forget(kind, value)

            try:
                with mock.patch.object(
                    session_module.threading,
                    "excepthook",
                ), mock.patch.object(
                    session_module,
                    "_forget_owned_record",
                    crash_after_slot_close,
                ):
                    store._close_tracked_claim_descriptor(
                        descriptor
                    )
                    deadline = time.monotonic() + 1
                    while not crashed:
                        if time.monotonic() >= deadline:
                            self.fail(
                                "worker forget interruption did not run"
                            )
                        time.sleep(0.005)
                    ...
                    while (
                        ...
                        if time.monotonic() >= deadline:
                            self.fail(
                                "consumed ownership was not forgotten"
                            )
                        time.sleep(0.005)
                ...
                while (
                    ...
                    if time.monotonic() >= deadline:
                        self.fail("consumed ownership was not forgotten")
                    time.sleep(0.005)
    ...
    def test_consumed_slot_survives_post_syscall_keyboard_interrupt(self):
        session_module = sys.modules["audit_session"]
        with tempfile.TemporaryDirectory() as temporary:
            store = self.module.SessionStore(Path(temporary))
            descriptor = store._open_directory(temporary)
            ...
            def close_then_interrupt(value):
                ...
                if not interrupted:
                    ...
                    self.assertEqual(replacement, [descriptor])
                    raise KeyboardInterrupt(
                        "signal delivered after close_range"
                    )
            ...
            try:
                with mock.patch.object(
                    ...
                    while not interrupted:
                        if time.monotonic() >= deadline:
                            self.fail(
                                "worker close interruption did not run"
                            )
                        time.sleep(0.005)
                    ...
                    while (
                        ...
                        if time.monotonic() >= deadline:
                            self.fail(
                                "quarantined ownership was not forgotten"
                            )
                        time.sleep(0.005)
                ...
                while (
                    ...
                    if time.monotonic() >= deadline:
                        self.fail("quarantined ownership was not forgotten")
                    time.sleep(0.005)
                os.fstat(replacement[0])
            finally:
                session_module._TRACKED_LEASE_OWNERSHIPS.discard(
    ...
    def test_request_close_hands_signal_safe_close_to_worker(self):
        with materialized_repo(
            "contract-compliant-overengineered"
        ) as repo, tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            original_close_range = session_module._CLOSE_RANGE
            ...
            class AsyncSignal(RuntimeError):
                pass

            def replace_then_interrupt(signum, frame):
                nonlocal interrupted
                os.dup2(replacement_source, descriptor)
                handler_seen.set()
                if not interrupted:
                    interrupted = True
                    raise AsyncSignal("post-close SIGUSR1")

            def close_then_signal(first, last, flags):
                result = original_close_range(first, last, flags)
                if first == descriptor and not close_threads:
                    ...
                    if not handler_seen.wait(1):
                        raise AssertionError(
                            "SIGUSR1 handler did not run"
                        )
                return result

            signal.signal(signal.SIGUSR1, replace_then_interrupt)
            ...
            try:
                with mock.patch.object(
                    ...
                    try:
                        lease.close()
                        while not handler_seen.wait(0.005):
                            if time.monotonic() - started_at > 1:
                                self.fail("worker close did not start")
                    except AsyncSignal:
                        pass
                    ...
                    while (
                        ...
                        if time.monotonic() >= deadline:
                            self.fail("worker cleanup did not finish")
                        time.sleep(0.005)
                ...
                self.assertIn(signal.SIGUSR1, worker_masks[0])
                os.fstat(descriptor)
            finally:
    ...
    def test_request_close_returns_while_worker_close_is_paused(self):
        with materialized_repo(
            "contract-compliant-overengineered"
        ) as repo, tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            try:
                with mock.patch.object(
                    ...
                    while (
                        ...
                        if time.monotonic() >= deadline:
                            self.fail("worker close did not finish")
                        time.sleep(0.005)
            finally:
                release_close.set()
                session_module._TRACKED_LEASE_OWNERSHIPS.discard(
    ...
    def test_consumed_slot_survives_each_bookkeeping_failure(self):
        ...
        class FailOnceSet(set):
            def __init__(self, values, *, fail_discard):
            ...
            def discard(self, value):
                super().discard(value)
                if self.fail_discard and not self.failed:
                    self.failed = True
                    raise KeyboardInterrupt("set bookkeeping interrupted")

        class FailOnceDict(dict):
            def __init__(self, values, *, fail_pop):
                super().__init__(values)
            ...
            def pop(self, key, default=None):
                ...
                if self.fail_pop and not self.failed:
                    self.failed = True
                    raise KeyboardInterrupt("ref bookkeeping interrupted")
                return result

        for failed_step in ("tracked", "pending", "lease-ref"):
            with self.subTest(failed_step=failed_step):
                ...
                original_refs = session_module._LEASE_REFS
                tracked = FailOnceSet(
                    {ownership},
                    fail_discard=failed_step == "tracked",
                )
                pending = FailOnceSet(
                    set(),
                    fail_discard=failed_step == "pending",
                )
                refs = FailOnceDict(
                    {ownership[1]: None},
                    fail_pop=failed_step == "lease-ref",
                )
                replacement = None
                close_calls = 0
                original_close_range = session_module._CLOSE_RANGE

                def count_close(first, last, flags):
                    nonlocal close_calls
                    close_calls += 1
                    return original_close_range(first, last, flags)

                session_module._TRACKED_LEASE_OWNERSHIPS = tracked
                session_module._PENDING_LEASE_CLOSE_FDS = pending
                session_module._LEASE_REFS = refs
                try:
                    with mock.patch.object(
                        session_module,
                        "_CLOSE_RANGE",
                    ...
                    self.assertNotIn(
                        ownership,
                        session_module
                        ._CONSUMED_DESCRIPTOR_OWNERSHIPS,
                    )
                finally:
                    session_module._TRACKED_LEASE_OWNERSHIPS = (
                        original_tracked
                    ...
                    session_module._LEASE_REFS = original_refs
                    (
                        session_module
                        ._CONSUMED_DESCRIPTOR_OWNERSHIPS
                        .discard(ownership)
                    ...
                    for value in (replacement, descriptor):
                        if value is None:
                            continue
                        try:
                            os.close(value)
    ...
    def test_direct_lease_keyboard_interrupt_queues_exact_cleanup(self):
        ...
        with mock.patch.object(
            ...
            while not interrupted:
                if time.monotonic() >= deadline:
                    self.fail("worker lease interruption did not run")
                time.sleep(0.005)
            ...
            while (
                ...
                if time.monotonic() >= deadline:
                    self.fail(
                        "interrupted lease close was not handed off"
                    )
                time.sleep(0.005)
    ...
    def test_tracked_claim_keyboard_interrupt_queues_exact_cleanup(self):
        ...
        with mock.patch.object(
            ...
            while not interrupted:
                if time.monotonic() >= deadline:
                    self.fail("worker tracked interruption did not run")
                time.sleep(0.005)
            ...
            while (
                ...
                if time.monotonic() >= deadline:
                    self.fail(
                        "interrupted tracked close was not handed off"
                    )
                time.sleep(0.005)
    ...
    def test_lease_finalizer_keyboard_interrupt_queues_exact_cleanup(self):
        ...
        with mock.patch.object(
            ...
            while not interrupted:
                if time.monotonic() >= deadline:
                    self.fail("worker finalizer interruption did not run")
                time.sleep(0.005)
            while ownership in session_module._TRACKED_LEASE_OWNERSHIPS:
                if time.monotonic() >= deadline:
        ...
        with materialized_repo(
            ...
            with mock.patch.object(
                ...
                with store._successor_transaction_lock(lease):
                    pass
                deadline = time.monotonic() + 1
                while interrupted_ownership is None:
                    if time.monotonic() >= deadline:
                        self.fail(
                            "worker transaction interruption did not run"
                        )
                    time.sleep(0.005)
    ...
    def test_transaction_publication_exit_crash_hands_off_exact_open(self):
        with materialized_repo(
            "contract-compliant-overengineered"
        ) as repo, tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            def interrupt_once(token):
                ...
                if not interrupted:
                    interrupted = True
                    raise KeyboardInterrupt(
                        "transaction publication interrupted"
                    )
                return original_exit(token)
            ...
            try:
                with mock.patch.object(
                    ...
                    with self.assertRaises(KeyboardInterrupt):
                        with store._successor_transaction_lock(
                            lease
                        ):
                            pass
                ...
                while (
                    ...
                    if time.monotonic() >= deadline:
                        self.fail(
                            "published transaction was not cleaned"
                        )
                    time.sleep(0.005)
                publisher = gate.try_enter_publication()
                self.assertIsNotNone(publisher)
                ...
                for publisher in tuple(gate._publishers):
                    if publisher.locked():
                        publisher.release()
                lease.close()
    ...
    def test_audit_reader_exit_keyboard_interrupt_releases_token(self):
        session_module = sys.modules["audit_session"]
        gate = session_module._FORK_FD_LIFECYCLE_GATE
        original_exit = gate.exit_audit
        interrupted = False

        def interrupt_once(reader):
            nonlocal interrupted
            if not interrupted:
                interrupted = True
                raise KeyboardInterrupt("reader exit interrupted")
            return original_exit(reader)
        ...
        try:
            ...
            self.assertFalse(
                any(reader.locked() for reader in gate._readers)
            )
            before = time.monotonic()
            gate.before_fork()
            elapsed = time.monotonic() - before
            ...
            for reader in tuple(gate._readers):
                if reader.locked():
                    reader.release()
            if gate._fork_pending:
                gate.after_parent_reopen()

    def test_one_shot_reader_release_is_nonblocking_with_waiter(self):
        session_module = sys.modules["audit_session"]
        token = session_module._OneShotToken()
    ...
    def test_worker_reader_exit_crash_still_allows_real_fork(self):
        ...
        def interrupt_once(reader):
            nonlocal crashed_worker, interrupted
            if not interrupted:
                interrupted = True
                crashed_worker = threading.current_thread()
                raise KeyboardInterrupt("worker reader exit interrupted")
            return original_exit(reader)
        ...
        try:
            with mock.patch.object(
                ...
                while (
                    ...
                    if time.monotonic() >= deadline:
                        self.fail("worker reader exit did not recover")
                    time.sleep(0.005)
    ...
    def test_tracked_publication_exit_crash_hands_off_exact_open(self):
        session_module = sys.modules["audit_session"]
        gate = session_module._DESCRIPTOR_OWNERSHIP_GATE
        original_exit = gate.exit_publication
        interrupted = False
        baseline = set(session_module._TRACKED_LEASE_OWNERSHIPS)

        def interrupt_once(token):
            nonlocal interrupted
            if not interrupted:
                interrupted = True
                raise KeyboardInterrupt("publication exit interrupted")
            return original_exit(token)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                with mock.patch.object(
                    ...
                    with self.assertRaises(KeyboardInterrupt):
                        store._open_tracked_claim_directory(
                            "claim",
                            dir_fd=parent,
                        )
                ...
                deadline = time.monotonic() + 1
                while (
                    set(session_module._TRACKED_LEASE_OWNERSHIPS)
                    != baseline
                ):
                    if time.monotonic() >= deadline:
                        self.fail("published claim open was not cleaned")
                    time.sleep(0.005)
                self.assertEqual(store._tracked_claim_ownerships, {})
                publisher = gate.try_enter_publication()
                self.assertIsNotNone(publisher)
                gate.exit_publication(publisher)
            finally:
    ...
    def test_claim_entry_crash_cleans_without_exception_gc(self):
        session_module = sys.modules["audit_session"]
        for entrypoint in ("lifecycle", "publication"):
            with self.subTest(entrypoint=entrypoint):
                retained_errors = []
                interrupted = False
                if entrypoint == "lifecycle":
                    gate = session_module._FORK_FD_LIFECYCLE_GATE
                    method = "try_enter_audit"
                else:
                    gate = session_module._DESCRIPTOR_OWNERSHIP_GATE
                    method = "try_enter_publication"
                original_enter = getattr(gate, method)
                ...
                def interrupt_once():
                    ...
                    if not interrupted:
                        interrupted = True
                        raise KeyboardInterrupt(
                            f"claim {entrypoint} entry interrupted"
                        )
    ...
    @unittest.skipUnless(hasattr(os, "fork"), "requires os.fork")
    def test_child_post_close_keyboard_interrupt_resets_state(self):
        with materialized_repo(
            "contract-compliant-overengineered"
        ) as repo, tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            parent_pid = os.getpid()
            interrupted = False
            snapshot_signals_blocked = False
            parent_signal_mask = signal.pthread_sigmask(
                signal.SIG_BLOCK,
                set(),
            )
            replacement_source = os.open(
            ...
            def close_then_interrupt(value):
                nonlocal interrupted, snapshot_signals_blocked
                if os.getpid() != parent_pid and not interrupted:
                    interrupted = True
                    snapshot_mask = signal.pthread_sigmask(
                        signal.SIG_BLOCK,
                        set(),
                    )
                    snapshot_signals_blocked = (
                        signal.SIGINT in snapshot_mask
                        and signal.SIGUSR1 in snapshot_mask
                    )
                    original_close(value)
                    os.dup2(
                        replacement_source,
            ...
            with mock.patch.object(
                ...
                if child == 0:
                    ...
                    result = (
                        ...
                        and snapshot_signals_blocked
                        and restored_signal_mask
        ...
        with materialized_repo(
            ...
            while True:
                try:
                    ...
                    break
                if time.monotonic() >= deadline:
                    self.fail("finalized lease cleanup did not finish")
                time.sleep(0.005)
            with self.assertRaises(OSError):
                os.fstat(descriptor)
            terminal_state = {
