---
name: check-data
description: "Audits the local database for the data needed to properly test the current branch's feature. Reads the branch diff + ticket to identify which tables/collections the feature touches, queries the DB to see what already exists, then produces a data-readiness plan covering happy-path, edge-case, error-path, and stupid-path (pathological input) rows. Writes a markdown report to `ai_docs/<branch>/data-plan.md` (or `.notes/<branch>/data-plan.md` if `.notes/` exists at the repo root) and creates the directory if missing. Use this skill whenever the user says 'check data', 'data readiness', 'is the db ready', 'what data do I need', 'plan seed data', 'data plan', '/check-data', 'check-data', or wants to plan what rows must exist in the DB before running QA. Also trigger when the user mentions 'audit the db for testing', 'data prep', 'do I have enough data to test this', or any DB-readiness check tied to a feature branch. Focus is data readiness only — does NOT generate test cases (use `/qa-ticket`) and does NOT insert data (use `/seed-data`). Supports Linear (default) and Jira — pass 'jira' as argument to use Jira (e.g., '/check-data jira')."
---
