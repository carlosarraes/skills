...
Write the report under `.notes/` when it exists, otherwise `ai_docs/`, in `<branch-name>/`. Every newly generated plan records the branch name, exact HEAD SHA, schema identity (migration/model fingerprint or "unknown"), and plan version. Use `data-plan.md` as the current plan. If it already exists, compare those metadata fields: current-plan reuse is allowed only when all match; otherwise preserve the old plan and write a new `data-plan-<short-sha>.md` candidate (reuse an existing candidate for that SHA) and select the new versioned candidate for the current run. There is no overwrite-approval prompt. Include ticket/platform/date, redacted DB connection, tables with before counts and roles, all four row buckets, insertion notes, and warnings. Raw credentials never appear in reports.

If `plan-only` was passed, stop immediately after writing or reusing the plan: report the path and **no database mutation**, and do not enter Steps 3–4. Otherwise, when the DB is reachable, continue from this plan in the same invocation without a second skill or confirmation. An unavailable DB takes the stop path above.

## Step 3: Seed data

Read the selected plan, including hand edits. The insertion preference is project seed/factory → ORM/REPL → direct DB/collection → API; fall through only when the preceding mechanism cannot cover the planned rows. Before invoking the selected seed mechanism, including a project seed/factory mechanism, inspect its scripts and configuration for destructive reset/drop/delete behavior, including truncation, or non-idempotent behavior. If it has either, do not invoke it. Select the next safe mechanism or stop and report when none is available. Do not use an API merely because a local DB is temporarily unreachable. For Mongo, use collections and document references; for multiple stores, keep a connection block per store.
